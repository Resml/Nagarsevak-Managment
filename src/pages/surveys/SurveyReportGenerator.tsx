...
export const SurveyReportGenerator: React.FC<SurveyReportGeneratorProps> = ({ surveys, onClose }) => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            pdf.save(`Surveys_Report_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
            onClose();
        } catch (error) {
            ...
            <div className="bg-white rounded-2xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
                <div className="flex justify-between items-center p-4 border-b bg-slate-50">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-lg font-bold text-slate-800">{t('surveys.title')} - {t('common.report_view')}</h2>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 bg-slate-100/50">
                    ...
                    >
                        <div className="report-header border-b-2 border-brand-600 pb-6 mb-8 flex justify-between items-end">
                            <div>
                                <h1 className="text-3xl font-bold text-brand-700 mb-1">{t('surveys.title')}</h1>
                                <p className="text-slate-500">{t('surveys.subtitle')}</p>
                            </div>
                            <div className="text-right text-sm text-slate-400">
                                <p>{t('common.date')}: {format(new Date(), 'dd/MM/yyyy')}</p>
                                <p>{t('surveys.found')}: {surveys.length}</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-[10px] table-fixed report-table-header">
                            <thead className="bg-slate-50">
                                <tr>
                                    ...
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[40%]">{t('surveys.table.title')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[15%]">{t('surveys.table.status')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[20%]">{t('surveys.table.questions')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[20%]">{t('surveys.table.created_at')}</th>
                                </tr>
                            </thead>
                        </table>

                        {surveys.map((survey, index) => (
                            <table key={survey.id} className="report-row w-full border-collapse border-b border-l border-r border-slate-200 text-[10px] table-fixed bg-white">
                                <tbody>
                                    <tr>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[5%] align-top">{index + 1}</td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[40%] align-top">
                                            <div className="font-bold text-slate-900"><TranslatedText text={survey.title} /></div>
                                            <div className="text-[9px] text-slate-500 mt-0.5 line-clamp-2"><TranslatedText text={survey.description} /></div>
                                            {survey.area && (
                                        ...
                                        <td className="border-r border-slate-200 px-2 py-2 w-[15%] align-top">
                                            <span className={`px-2 py-1 rounded-full text-[8px] font-medium ${survey.status === 'Active' ? 'bg-green-100 text-green-700' :
                                        ...
                                        <td className="border-r border-slate-200 px-2 py-2 w-[20%] align-top">
                                            <div className="font-mono text-slate-700">{survey.questions?.length || 0} {t('surveys.questions_suffix')}</div>
                                        </td>
                                        <td className="px-2 py-2 w-[20%] align-top text-slate-600">
