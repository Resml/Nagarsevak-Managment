        ...
        <div className="space-y-6 flex flex-col h-full">
            <div className="flex justify-between items-start flex-none">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Digital meeting space for voters and citizens.</p>
                </div>
            ...
            <div className="flex space-x-1 bg-white p-1 rounded-xl border border-gray-200 flex-none w-fit">
                <button onClick={() => setActiveTab('rooms')} className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'rooms' ? 'bg-brand-50 text-brand-700 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}>Active Rooms</button>
                <button onClick={() => setActiveTab('history')} className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'history' ? 'bg-brand-50 text-brand-700 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}>Past Meetings</button>
            </div>

            <div className="flex-1 overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {rooms.map(room => (
                        <div key={room.id} className="ns-card p-6 flex flex-col justify-between hover:border-orange-200 transition-all border-l-4 border-l-orange-500">
                            <div>
                            ...
                            <button className="w-full py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-lg transition-all flex items-center justify-center gap-2">
                ...
                {rooms.length === 0 && (
                    <div className="text-center py-20 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                        ...
                        <h3 className="text-slate-900 font-bold">No rooms active</h3>
                        <p className="text-slate-500 text-sm">Create a new room to start a meeting.</p>
                    </div>
                )}
            </div>
        </div>
