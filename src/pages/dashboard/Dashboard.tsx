    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-3">
                    ...
                    <button className="bg-brand-600 text-white px-4 py-2 rounded-lg font-medium shadow-sm hover:bg-brand-700 transition flex items-center gap-2">
                        <Megaphone className="w-4 h-4" /> {t('dashboard.broadcast')}
                    </button>
                </div>
            </div>
            ...
            {dailyBriefing && (
                <div className="bg-white border-l-4 border-brand-500 rounded-r-xl p-6 shadow-sm relative overflow-hidden">
                    <div className="relative z-10 flex gap-4 items-start">
                        ...
                        <div>
                            <h3 className="font-semibold text-lg text-gray-900 mb-1">{t('dashboard.ai_insight')}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title={t('dashboard.total_citizens')} value={stats.voters} icon={Users} color="text-blue-600" trend="up" />
                <StatCard title={t('dashboard.pending_issues')} value={stats.pending} icon={AlertCircle} color="text-red-600" trend="up" />
                <StatCard title={t('dashboard.in_progress')} value={stats.inProgress} icon={Activity} color="text-yellow-600" trend="down" />
                <StatCard title={t('dashboard.resolved')} value={stats.resolved} icon={CheckCircle} color="text-green-600" trend="up" />
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-lg font-bold text-gray-900">{t('dashboard.status_overview')}</h2>
                        <button onClick={() => navigate('/complaints')} className="text-sm text-brand-600 font-medium hover:underline">{t('dashboard.view_all')}</button>
                    </div>

                    {/* Custom CSS Bar Chart */}
                    <div className="space-y-6">
                        {[
                            { label: t('dashboard.pending_issues').replace('Pending Issues', 'Pending'), count: stats.pending, color: 'bg-red-500', width: `${(stats.pending / stats.total) * 100}%` },
                            { label: t('dashboard.in_progress'), count: stats.inProgress, color: 'bg-yellow-500', width: `${(stats.inProgress / stats.total) * 100}%` },
                            { label: t('dashboard.resolved'), count: stats.resolved, color: 'bg-green-500', width: `${(stats.resolved / stats.total) * 100}%` },
                        ].map((item: any) => (
                            <div key={item.label}>
                    ...
                    <div className="mt-8 grid grid-cols-3 gap-4 border-t border-gray-100 pt-6">
                        <div className="text-center">
                            <p className="text-xs text-gray-500 uppercase">{t('dashboard.clearance_rate')}</p>
                            <p className="text-xl font-bold text-gray-900 mt-1">
                                {stats.total > 0 ? Math.round(((stats.resolved) / stats.total) * 100) : 0}%
                            </p>
                        </div>
                        <div className="text-center border-l border-gray-100">
                            <p className="text-xs text-gray-500 uppercase">{t('dashboard.avg_response')}</p>
                            <p className="text-xl font-bold text-gray-900 mt-1">4.2 Hrs</p>
                        </div>
                        <div className="text-center border-l border-gray-100">
                            <p className="text-xs text-gray-500 uppercase">{t('dashboard.satisfaction')}</p>
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 overflow-hidden flex flex-col">
                    <h2 className="text-lg font-bold text-gray-900 mb-4">{t('dashboard.live_activity')}</h2>
                    <div className="flex-1 overflow-y-auto pr-2 space-y-4 max-h-[400px]">
                        ...
                        {recentActivity.length === 0 && (
                            <p className="text-sm text-gray-400 text-center py-8">{t('dashboard.no_activity')}</p>
                        )}
                    </div>
