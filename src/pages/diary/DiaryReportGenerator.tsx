...
interface DiaryReportGeneratorProps {
    entries: DiaryEntry[];
    onClose: () => void;
}

export const DiaryReportGenerator: React.FC<DiaryReportGeneratorProps> = ({ entries, onClose }) => {
    ...
    const getStatusLabel = (status: string) => {
        if (language === 'mr') {
            switch (status) {
                case 'Raised': return 'मांडलेला';
                case 'In Discussion': return 'चर्चेत';
                case 'Action Taken': return 'कार्यवाही झाली';
    ...
    const getMeetingTypeLabel = (type: string) => {
        if (language === 'mr') {
            switch (type) {
                case 'GB': return 'जीबी';
                case 'Standing Committee': return 'स्थायी समिती';
                case 'Ward Committee': return 'वॉर्ड समिती';
                case 'Special GB': return 'विशेष जीबी';
                case 'Other': return 'इतर';
                default: return type;
            }
        }
        return type;
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            pdf.save(`GB_Register_Report_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
            onClose();
        } catch (error) {
                ...
                <div className="flex-1 overflow-y-auto p-8 bg-slate-100/50">
                    ...
                    >
                        <div className="report-header border-b-2 border-brand-600 pb-6 mb-8 flex justify-between items-end">
                            <div>
                                <h1 className="text-3xl font-bold text-brand-700 mb-1">{language === 'mr' ? 'GB रजिस्टर' : 'GB Register'}</h1>
                                <p className="text-slate-500">{t('diary.subtitle') || (language === 'mr' ? 'सभा नोंदी आणि ठराव' : 'Meeting records and resolutions')}</p>
                            </div>
                            <div className="text-right text-sm text-slate-400">
                                <p>{t('common.report_columns.date')}: {format(new Date(), 'dd/MM/yyyy')}</p>
                                <p>{language === 'mr' ? 'एकूण नोंदी' : 'Total Records'}: {entries.length}</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-[10px] table-fixed report-table-header">
                            <thead className="bg-slate-50">
                                <tr>
                                    ...
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[12%]">{t('common.report_columns.date')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[12%]">{t('common.report_columns.meeting_type')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[27%]">{t('common.report_columns.subject')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[15%]">{t('common.report_columns.department')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[14%]">{t('common.report_columns.outcome')}</th>
                        ...
                        {entries.map((entry, index) => (
                            <table key={entry.id} className="report-row w-full border-collapse border-b border-l border-r border-slate-200 text-[10px] table-fixed bg-white">
                                <tbody>
                                    <tr>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[5%] align-top">{index + 1}</td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[12%] align-top">
                                            {format(new Date(entry.meetingDate), 'dd/MM/yyyy')}
                                        </td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[12%] align-top font-medium">
                                            {getMeetingTypeLabel(entry.meetingType)}
                                        </td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[27%]">
                                            <div className="font-bold"><TranslatedText text={entry.subject} /></div>
                                            {entry.description && (
                                                <div className="text-[8px] text-slate-500 mt-0.5 line-clamp-2"><TranslatedText text={entry.description} /></div>
                                            )}
                                        </td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[15%] align-top">
                                            <TranslatedText text={entry.department || '--'} />
                                        </td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[14%] align-top">
                                            <span className={`px-1 py-0.5 rounded text-[8px] font-bold border inline-block ${
                                                entry.status === 'Action Taken' ? 'bg-green-50 text-green-700 border-green-200' :
                                                entry.status === 'In Discussion' ? 'bg-blue-50 text-blue-700 border-blue-200' :
