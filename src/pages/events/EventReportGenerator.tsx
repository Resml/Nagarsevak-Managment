...
import { X, Download, FileText, Clock, MapPin } from 'lucide-react';
...
export const EventReportGenerator: React.FC<EventReportGeneratorProps> = ({ events, onClose }) => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            pdf.save(`Events_Report_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
            onClose();
        } catch (error) {
            ...
            <div className="bg-white rounded-2xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
                <div className="flex justify-between items-center p-4 border-b bg-slate-50">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-lg font-bold text-slate-800">{t('events.title')} - {t('common.report_view')}</h2>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 bg-slate-100/50">
                    ...
                    >
                        <div className="report-header border-b-2 border-brand-600 pb-6 mb-8 flex justify-between items-end">
                            <div>
                                <h1 className="text-3xl font-bold text-brand-700 mb-1">{t('events.title')}</h1>
                                <p className="text-slate-500">{t('events.subtitle')}</p>
                            </div>
                            <div className="text-right text-sm text-slate-400">
                                <p>{t('common.date')}: {format(new Date(), 'dd/MM/yyyy')}</p>
                                <p>{t('events.found')}: {events.length}</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-[10px] table-fixed report-table-header">
                            <thead className="bg-slate-50">
                                <tr>
                                    ...
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[20%]">{t('common.report_columns.date_time')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[30%]">{t('common.report_columns.title_type')}</th>
                                    ...
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[20%]">{t('events.audience_label')}</th>
                                </tr>
                            </thead>
                        </table>

                        {events.map((event, index) => (
                            <table key={event.id} className="report-row w-full border-collapse border-b border-l border-r border-slate-200 text-[10px] table-fixed bg-white">
                                <tbody>
                                    <tr>
                                        ...
                                        <td className="border-r border-slate-200 px-2 py-2 w-[20%] align-top">
                                            <div className="font-bold text-slate-900">{format(new Date(event.event_date), 'MMM d, yyyy')}</div>
                                            <div className="text-[9px] text-slate-500 mt-0.5">{event.event_time}</div>
                                        </td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[30%] align-top">
                                            <div className="font-bold text-slate-900"><TranslatedText text={event.title} /></div>
                                            <div className="text-[9px] text-slate-500 mt-0.5">{event.type}</div>
                                        </td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[25%] align-top">
                                            <div className="font-semibold text-slate-800"><TranslatedText text={event.area || 'N/A'} /></div>
                                            <div className="text-[9px] text-slate-500 mt-0.5"><TranslatedText text={event.location} /></div>
                                        </td>
                                        <td className="px-2 py-2 w-[20%] align-top">
                                            {event.target_audience === 'All' ? (
                                                <span className="text-[9px] text-slate-500">{t('events.audiences.All') || 'All Citizens'}</span>
                                            ) : (
                                                <span className="text-[9px] font-bold text-orange-800 px-1 py-0.5 rounded bg-orange-100">
