...
interface StaffReportGeneratorProps {
    staffList: Staff[];
    activeTab: string;
    onClose: () => void;
}

export const StaffReportGenerator: React.FC<StaffReportGeneratorProps> = ({ staffList, activeTab, onClose }) => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            pdf.save(`Staff_Report_${activeTab}_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
            onClose();
        } catch (error) {
                    ...
                    >
                        <div className="report-header border-b-2 border-brand-600 pb-6 mb-8 flex justify-between items-end">
                            ...
                            <div className="text-right text-sm text-slate-400">
                                ...
                                <p>{t('staff.list.found')}: {staffList.length}</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-[10px] table-fixed report-table-header">
                            <thead className="bg-slate-50">
                                <tr>
                                    ...
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[30%]">{t('common.report_columns.name_role')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[20%]">{t('common.report_columns.contact')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[15%]">{t('common.report_columns.area')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[30%]">{t('common.report_columns.keywords_categories')}</th>
                                </tr>
                            </thead>
                        </table>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td className="border-r border-slate-200 px-2 py-2 w-[30%] align-top">
                                            <div className="font-bold text-slate-900"><TranslatedText text={member.name} /></div>
                                            <div className="text-[9px] text-slate-500 mt-0.5"><TranslatedText text={member.role} /></div>
                                        </td>
                                        ...
                                        <td className="border-r border-slate-200 px-2 py-2 w-[15%] align-top">
                                            {member.area ? (
                                                ...
                                                <div className="text-slate-400 italic">N/A</div>
                                            )}
                                        </td>
                                            ...
                                            <div className="flex flex-wrap gap-1">
                                                {member.keywords && member.keywords.length > 0 ? (
                                                    member.keywords.map((k, i) => (
                                                        <span key={i} className="inline-block bg-blue-50 text-blue-700 border border-blue-100 px-1 py-0.5 rounded text-[8px] font-medium mr-1 mb-1">
                                                            <TranslatedText text={k} />
                                                        </span>
                                                    ))
                                                ) : (
                                                    <span className="text-slate-400 italic text-[9px]">{t('staff.list.no_keywords')}</span>
                                                )}
                                            </div>
                                        </td>
                                    </tr>
