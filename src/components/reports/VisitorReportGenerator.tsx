...
interface VisitorReportGeneratorProps {
    visitors: any[];
    onClose: () => void;
}

export const VisitorReportGenerator: React.FC<VisitorReportGeneratorProps> = ({ visitors, onClose }) => {
    const { t } = useLanguage();
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            pdf.save(`Visitor_Log_Report_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
            onClose();
        } catch (error) {
                    ...
                    >
                        <div className="report-header border-b-2 border-brand-600 pb-6 mb-8 flex justify-between items-end">
                            ...
                            <div className="text-right text-sm text-slate-400">
                                ...
                                <p>Total Records: {visitors.length}</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-[10px]">
                            <thead className="report-table-header">
                                <tr className="bg-slate-50">
                                    ...
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[20%]">{t('office.full_name')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[15%]">{t('office.mobile')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[20%]">{t('office.area')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[15%]">{t('office.purpose')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[15%]">{t('office.reference')}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {visitors.map((visitor) => (
                                    <tr key={visitor.id} className="report-row border-b border-slate-100">
                                        <td className="border border-slate-200 px-2 py-2 w-[15%] align-top">
                                            {format(new Date(visitor.visit_date), 'dd/MM/yyyy')}
                                            <br />
                                            <span className="text-[8px] text-slate-400">{format(new Date(visitor.visit_date), 'hh:mm a')}</span>
                                        </td>
                                        <td className="border border-slate-200 px-2 py-2 w-[20%] font-bold align-top">
                                            <TranslatedText text={visitor.name || '-'} />
                                        </td>
                                        <td className="border border-slate-200 px-2 py-2 w-[15%] font-mono align-top">
                                            {visitor.mobile || '-'}
                                        </td>
                                        <td className="border border-slate-200 px-2 py-2 w-[20%] align-top">
                                            <TranslatedText text={visitor.area || '-'} />
                                        </td>
                                        <td className="border border-slate-200 px-2 py-2 w-[15%] align-top">
                                            <TranslatedText text={visitor.purpose || '-'} />
                                        </td>
                                        <td className="border border-slate-200 px-2 py-2 w-[15%] align-top">
                                            <TranslatedText text={visitor.reference || '-'} />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
