...
interface SchemeListReportGeneratorProps {
    schemes: Scheme[];
    onClose: () => void;
    title?: string;
    subtitle?: string;
...
export const SchemeListReportGenerator: React.FC<SchemeListReportGeneratorProps> = ({ schemes, onClose, title, subtitle }) => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            pdf.save(`Schemes_Report_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
            onClose();
        } catch (error) {
                ...
                <div className="flex-1 overflow-y-auto p-8 bg-slate-100/50">
                    ...
                    >
                        <div className="report-header border-b-2 border-brand-600 pb-6 mb-8 flex justify-between items-end">
                            <div>
                                <h1 className="text-3xl font-bold text-brand-700 mb-1">{title || t('schemes.title') || 'Government Schemes'}</h1>
                                <p className="text-slate-500">{subtitle || t('schemes.subtitle')}</p>
                            </div>
                            <div className="text-right text-sm text-slate-400">
                                <p>{t('common.report_columns.date')}: {format(new Date(), 'dd/MM/yyyy HH:mm')}</p>
                                <p>{t('work_history.found') || 'Found'}: {schemes.length}</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-[10px] table-fixed report-table-header">
                            <thead className="bg-slate-50">
                                <tr>
                                    ...
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[20%]">{t('common.report_columns.scheme_name')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[30%]">{t('common.report_columns.description')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[15%]">{t('common.report_columns.category')}</th>
                                    <th className="border border-slate-200 px-2 py-2 text-left font-bold text-slate-700 w-[30%]">{t('common.report_columns.benefits')}</th>
                                </tr>
                            </thead>
                        </table>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td className="border-r border-slate-200 px-2 py-2 w-[20%] align-top">
                                            <div className="font-bold text-slate-800">{getLocalizedData(item.name)}</div>
                                        </td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[30%] align-top">
                                            <div className="text-slate-600 line-clamp-4">{getLocalizedData(item.description)}</div>
                                        </td>
                                        <td className="border-r border-slate-200 px-2 py-2 w-[15%] align-top font-medium text-slate-700">
                                            {item.category ? (t(`common.${item.category}`) || item.category) : '-'}
                                        </td>
                                        <td className="px-2 py-2 w-[30%] align-top">
                                            <div className="text-slate-600 line-clamp-4">{getLocalizedData(item.benefits)}</div>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
