...
for r in rows:
    ...
    # NEW EXCLUSIONS (The 7 legitimate ones)
    ...
    # ASSUME THESE 4 ARE DROPPED (As per Test 19 cleanup)
    if table == 'incoming_letters' and policy in ('Allow authenticated users to insert incoming letters', 'Allow users to update own incoming letters'):
        continue
    if table == 'event_rsvps' and policy in ('Enable insert access for authenticated users', 'Enable update access for authenticated users'):
        continue
...
for i, r in enumerate(rogues, 1):
    if "anon" in r['policyname'].lower() or "public" in r['policyname'].lower():
        pass # Already dropped in previous migrations
    else:
        true_rogues += 1
        print(f"ROGUE -> {r['tablename']} | {r['policyname']} | {r.get('operation')} | {r.get('roles')}")

print(f"Net remaining rogues after dropping the 4: {true_rogues}")
