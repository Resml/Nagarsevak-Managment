...
TRUNCATE public.phase5b_verify_results;
...
BEGIN
    ...
    IF v_count = 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (1, 'FAIL', 'has_member_feature_access function is MISSING');
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (1, 'PASS', 'has_member_feature_access function exists');
    ...
    IF v_count = 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (2, 'FAIL', 'trg_validate_staff_permissions trigger is MISSING');
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (2, 'PASS', 'trg_validate_staff_permissions trigger exists');
    ...
    IF v_count = 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (3, 'FAIL', 'trg_prevent_staff_permission_escalation trigger is MISSING');
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (3, 'PASS', 'trg_prevent_staff_permission_escalation trigger exists');
    ...
    IF v_count < 2 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (4, 'FAIL', 'Staff INSERT/UPDATE policies do not contain has_member_feature_access check in WITH CHECK or USING clause');
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (4, 'PASS', 'Staff INSERT/UPDATE policies use has_member_feature_access');
    ...
    IF v_count = 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (5, 'FAIL', 'Staff DELETE policy does not enforce tenant isolation in USING clause');
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (5, 'PASS', 'Staff DELETE policy is tenant-isolated');
    ...
    IF v_count < 3 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (6, 'FAIL', REPLACE('Expected 3 SECURITY DEFINER functions with search_path=public, found %', '%', (v_count)::text));
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (6, 'PASS', 'All 3 SECURITY DEFINER functions have search_path=public');
    ...
    -- -------------------------------------------------------------------------
        ...
        IF v_overload_count = 0 THEN
            INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (7, 'FAIL', 'has_member_feature_access not found in pg_proc -- function is missing!');
        END IF;

        RAISE NOTICE 'TEST 7 DIAGNOSTIC: Total overloads found: %', v_overload_count;

        IF v_any_anon_exec THEN
            INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (7, 'FAIL', 'anon has EXECUTE on at least one overload of has_member_feature_access. See WARNING above for the exact REVOKE SQL. Apply phase5b_revoke_fix.sql after verifying the canonical signature matches.');
        END IF;

        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (7, 'PASS', 'No overload of has_member_feature_access is executable by anon');
    ...
    IF v_count < 56 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (8, 'FAIL', REPLACE('Expected >=56 Phase 5B secure INSERT/UPDATE policies, found %', '%', (v_count)::text));
    END IF;
    ...
    IF v_count < 28 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (9, 'FAIL', REPLACE('Cross-tenant isolation AND feature gate logic missing -- only % of 28 INSERT policies contain proper rules in with_check', '%', (v_count)::text));
    END IF;
    ...
    IF v_count < 28 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (9, 'FAIL', REPLACE('Cross-tenant isolation AND feature gate logic missing -- only % of 28 UPDATE policies contain proper rules in qual', '%', (v_count)::text));
    END IF;

    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (9, 'PASS', 'Cross-tenant isolation logic and feature gates intact in 28 INSERT and 28 UPDATE policies');
    ...
    IF v_missing_policies != '' THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (10, 'FAIL', REPLACE('Missing has_member_feature_access in SELECT policies for: [%]', '%', (trim(v_missing_policies))::text));
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (10, 'PASS', 'All 27 feature tables have has_member_feature_access in their SELECT policy');
    ...
    IF v_count = 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (11, 'FAIL', 'Phase 3B anonymous survey policies are MISSING');
    END IF;
    ...
    IF v_count = 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (12, 'FAIL', 'Phase 3 Storage policies are MISSING');
    END IF;
    ...
    IF v_count > 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (13, 'FAIL', REPLACE('whatsapp_sessions has % unexpected public policies -- should be 0 (service_role only)', '%', (v_count)::text));
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (13, 'PASS', 'whatsapp_sessions has 0 public policies (service_role only)');
    ...
    IF v_count = 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (14, 'FAIL', 'Phase 4 has_feature_access function is MISSING');
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (14, 'PASS', 'has_feature_access (Phase 4) remains intact');
    ...
    IF v_count > 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (15, 'FAIL', REPLACE('Found % legacy complaint/VA policies that bypass Phase 5B entitlement', '%', (v_count)::text));
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (15, 'PASS', '0 legacy complaint/VA bypass policies remain');
    ...
    IF v_count > 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (16, 'FAIL', REPLACE('Found % legacy insecure duplicate staff policies', '%', (v_count)::text));
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (16, 'PASS', '0 legacy insecure duplicate staff policies remain');
    ...
    IF v_count > 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (17, 'FAIL', REPLACE('Found % permissive legacy "Tenant Insert/Update <table>" policies that bypass the feature gate', '%', (v_count)::text));
    END IF;
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (17, 'PASS', '0 permissive legacy Phase 2 "Tenant Insert/Update" bypass policies remain for the 28 target tables');
    ...
    IF v_count != 28 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (18, 'FAIL', REPLACE('Expected exactly 28 secure INSERT policies containing has_member_feature_access, found %', '%', (v_count)::text));
    ELSE
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (18, 'PASS', 'exactly 28 secure INSERT policies found for target tables');
    END IF;
    ...
    IF v_count != 28 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (18, 'FAIL', REPLACE('Expected exactly 28 secure UPDATE policies containing has_member_feature_access, found %', '%', (v_count)::text));
    ELSE
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (18, 'PASS', 'exactly 28 secure UPDATE policies found for target tables');
    END IF;
    ...
    IF v_count > 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (19, 'FAIL', REPLACE('Found % remaining extra/rogue INSERT/UPDATE policies on target tables.', '%', (v_count)::text));
    ELSE
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (19, 'PASS', 'No remaining rogue INSERT/UPDATE permissive policies on target tables');
    END IF;
    ...
    IF v_count != 1 THEN 
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (20, 'FAIL', REPLACE('Auth Survey Insert (or legacy Enable insert for authenticated users) must exist exactly once on survey_responses as an INSERT, found %', '%', (v_count)::text)); 
    END IF;
    ...
    IF v_count > 0 THEN 
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (20, 'FAIL', 'survey_responses exception policies found on other target tables'); 
    END IF;
    
    INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (20, 'PASS', 'Phase 3B survey_responses exceptions properly verified');
    ...
    WHERE schemaname = 'public'
      ...
      AND (qual LIKE '%has_member_feature_access%' OR qual LIKE '%user_tenant_mapping%' OR qual LIKE '%tenant_id%');
      
    -- We expect 9 tables * 2 policies (SELECT and DELETE) = 18 policies
    IF v_count < 18 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (21, 'FAIL', REPLACE('Expected 18 Tenant-specific/Unified Select/Delete policies for the legacy ALL tables, found %. All must contain tenant isolation and feature access.', '%', (v_count)::text));
    ELSE
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (21, 'PASS', 'All 18 Tenant-specific/Unified Select/Delete replacements are in place and secure.');
    END IF;
    ...
    IF v_count = 0 THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (21, 'FAIL', 'Anon Survey Select policy is required for public survey forms.');
    ELSE
        INSERT INTO public.phase5b_verify_results (test_number, status, message) VALUES (21, 'PASS', 'Anon Survey Select policy is present.');
    END IF;
...
-- We append a final aggregate result row so it is visible in the SELECT output.
-- We must NOT raise an exception here, otherwise Supabase rolls back the entire script 
-- execution and hides the SELECT output!
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM public.phase5b_verify_results WHERE status = 'FAIL') THEN
        INSERT INTO public.phase5b_verify_results (test_number, status, message) 
        VALUES (999, 'FAIL', 'PHASE 5B VERIFICATION FAILED: One or more tests failed. Check the results above.');
    ELSE
        INSERT INTO public.phase5b_verify_results (test_number, status, message) 
        VALUES (999, 'PASS', 'PHASE 5B VERIFICATION PASSED COMPLETELY.');
    END IF;
END $$;

SELECT test_number, status, message 
FROM public.phase5b_verify_results 
