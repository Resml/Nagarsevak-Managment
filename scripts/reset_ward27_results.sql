-- Clean up old election results and ensure Ward 27 results are showing
-- Tenant ID: bf1a3e36-464e-4eff-b21d-dc71f5a5a582

-- Step 1: Delete ALL existing election results for this tenant
DELETE FROM election_results 
WHERE tenant_id = 'bf1a3e36-464e-4eff-b21d-dc71f5a5a582';

-- Step 2: Insert fresh Ward 27 results (A, B, C, D)
...
-- Verification: Show what's in the database now
SELECT ward_name, booth_number, booth_name, winner, margin, total_votes_casted
...
DO $$
BEGIN
    RAISE NOTICE 'Old results deleted. Ward 27 results (A, B, C, D) imported successfully!';
    RAISE NOTICE 'Total records for this tenant: 4';
END $$;
